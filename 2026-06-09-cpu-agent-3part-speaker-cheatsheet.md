...
| **P1 基础** | 8 min | 强调**loop 本质是 compute↔comm 乒乓**：用一个真实 turn 的 9 步轨迹（[call_llm ~570ms, CPU~0%] → read_file(fork 6.5ms) → run_tests 占 35-61% → answer）把抽象循环变具体；再用"手搓 agent"代码（`for step in range(250)` + call_llm + TOOLS + ctx 累积）点明复杂度在通信↔计算交替、不在单步。 |
...
**Q5：手搓一个最简 agent 到底要写哪些代码？复杂吗？**
答：不复杂——核心就是**一个 `for` 循环 + 一个远程调用 + 几个本地函数 + ctx 累积**（见 P1-3 代码）：`for step in range(250)`（SWE-bench 上限 250 步/$3 ✅）里，`call_llm(ctx)` 是唯一的远程通信边界（~570ms、等待期 CPU~0% ⚠️），`TOOLS[name](**args)` 是本地计算（fork/exec，占请求 35-61% ✅），`ctx += […]` 是每轮的 JSON 序列化税（simdjson 靶点）。复杂度不在任何一步，而在**循环里通信与计算的交替结构**——这就是我们系统软件的主场。
