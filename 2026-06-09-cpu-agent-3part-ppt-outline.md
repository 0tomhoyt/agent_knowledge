...
- **一个真实 turn 的 9 步执行轨迹**（修一个测试错误）：感知 → [call_llm ~570ms, CPU~0%] → read_file(fork 6.5ms) → 记忆更新 → [call_llm] → run_tests(占请求 35-61%) → answer——计算与通信反复乒乓（P1-4 主线雏形）
...
- **手搓最简 agent**（代码视角）：整个循环 ≈ 一个 `for step in range(250)` + `call_llm(ctx)`（远程通信，~570ms，等待期 CPU~0%）+ `TOOLS[name](**args)`（本地计算，fork/exec，占请求 35-61%）+ `ctx += …`（JSON 序列化税，simdjson 靶点）——复杂度不在单步，而在通信↔计算的交替结构
...
### 【第17页】算子侧：小批量 Agent kernel on ARM SIMD
...
### 【第19页】融合点：同一组单元、同一份布局（punchline）
