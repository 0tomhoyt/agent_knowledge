...
| 5 | API 等待期间本地 CPU 利用率 **~0%**（⚠️ 数量级估算，串行阻塞场景）[来源: 承袭旧详细文档 P1-4] | "在等 API 的 500 毫秒到 5 秒里，本地 CPU 利用率接近 0%——注意这是数量级估算，前提是串行阻塞场景。" |
| 6 | 多 Agent 比单 Agent 贵约 **2-5×** token/成本（⚠️ 经验法则/数量级估算，指 token-or-cost 而非 total-time，受任务/AI 数/轮数影响）[来源: 承袭旧详细文档 P2-3，综合 UIUC arXiv:2505.18286] | "多 Agent 比单 Agent 贵约 2 到 5 倍——但这是 token/成本的经验法则，不是总时间测量，随任务、agent 数、轮数变化。" |
